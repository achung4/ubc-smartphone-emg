 ...
 */
public class NewRecordingVideoActivity extends OrmLiteBaseActivity<DatabaseHelper> implements android.widget.PopupMenu.OnMenuItemClickListener, OnSharedPreferenceChangeListener {


    public double cal_num = 0;
    public boolean first = true;

    private static final String TAG = NewRecordingVideoActivity.class.getName();
...
//				saveRecordingOnInternalDB();
    ...
    private void sendRecordingDuration() {
        if (isServiceBounded && serviceMessenger != null) {
			...
			System.out.println("##### NewRecordingVideoActivity ##### - Duration: " + duration);
			msg.setData(extras);
			msg.replyTo = activityMessenger;
			try {
				serviceMessenger.send(msg);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_new_recording_video);
        Log.i(TAG, "onCreate()");
...
//		uiRecordingName.setText(recording.getName());
    ...
    /**
     * Releases the Media Recorder object
     */
    ...
    /**
     * Setting the camera instance of the MediaReocrder
     * Setting the audio and video source
     * Setting the video output type at "QUALITY_720P"
     * and setting the output file with max duration and video file size.
    ...
    private boolean prepareMediaRecorder() {
        ...
        //mediaRecorder.setOutputFile("/sdcard/"+ name + time + ".mp4");
        //mediaRecorder.setOutputFile(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM).toString() + "/" + name + time + ".mp4");
        //mediaRecorder.setOutputFile(Environment.getExternalStorageDirectory().toString() + Constants.APP_DIRECTORY + "/" + name + time + ".mp4");
    ...
    protected void onResume() {
        ...
        // VIDEO
        // checks if your phone has a camera on resume
        if(!checkCameraHardware(classContext)){
            Toast toast = Toast.makeText(classContext, "Sorry, your phone does not have a camera!", Toast.LENGTH_SHORT);
            toast.show();
            finish();
        }
        Log.i(TAG, "onResume()");
        ...
        Thread connectionThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if(!prepareMediaRecorder()){
                    Toast.makeText(NewRecordingVideoActivity.this, "Fail in prepareMediaRecorder()!\n - Ended -", Toast.LENGTH_LONG).show();
                    finish();
                }
...
//                            }
    ...
    private void stopChronometer() {
        ...
        System.out.println("##### NewRecordingVideoActivity ##### - Duration of recording is: " + this.duration);
    }
    ...
    public void onMainButtonClicked(View view) {
        // Starts recording
        System.out.println("##### NewRecordingVideoActivity ##### - Main Button is Clicked");
